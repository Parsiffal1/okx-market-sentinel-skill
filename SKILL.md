...
description: |
  OKX 市场哨兵：当用户要做市场监控、持仓优先复查、热点标的筛选、事件驱动解释、Telegram 风格简报或风险摘要时使用。
  这个 skill 教 agent 用“信息分层 → 持仓优先 → 风险归因 → 结果排序 → 简洁输出”的方式处理 OKX 可交易市场。
...
这个 skill 的价值不在于“多抓几个数据源”，而在于让 agent 学会三件事：

1. **先看什么** —— 当前持仓、当前风险、当前最活跃的标的
2. **怎么判断** —— 宏观驱动、加密原生驱动、结构确认、热度噪音分别处理
3. **怎么表达** —— 输出一份人和下游 agent 都能直接使用的摘要
