...
这个 skill 的价值不在于“多抓几个数据源”，而在于把交易者真正需要的市场判断层沉淀下来。

它教 agent 学会四件事：

1. **先看什么** —— 当前持仓、当前风险、当前最活跃的标的
2. **怎么判断** —— 宏观驱动、加密原生驱动、结构确认、热度噪音分别处理
3. **怎么交付** —— 输出一份交易者和其他使用者都能直接使用的摘要
4. **怎么复用** —— 如果需要，也可以被更大的 trading workflow 或 agent system 接进去

如果把普通行情抓取看成“拿信息”，这个 skill 做的是：

> **把信息变成判断，把判断变成可直接使用的结果。**
...
触发与升级规则见：`references/trigger-policy.md`
